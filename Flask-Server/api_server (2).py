...
def get_top10():
    ...
    # Ultra-simple query that matches the working full roster query
    query = """
        SELECT 
...
def get_game_champions():
    ...
    # Get max score per machine first, then join to get the player name
    query = """
        WITH max_scores AS (
            SELECT 
                machine_id,
        ...
        FROM max_scores ms
        JOIN high_scores_archive h ON h.machine_id = ms.machine_id 
            AND h.high_score = ms.max_score
            AND h.event_code = %s
    ...
    results = query_db(query, (event_code, event_code))
    if results is None or len(results) == 0:
        print("⚠️ Game champions query returned None or empty")
        print(f"   Event code used: {event_code}")
        return jsonify([])
